   ...
   m_ecnCWRSeq(sock.m_ecnCWRSeq)
{
 ...
 if (sock.m_congestionControl)
 {
     ...
     if(ppt){
        ppt->Init(m_tcb, m_rtt->GetEstimate().GetSeconds());
     }
     else  m_congestionControl->Init(m_tcb);
 }
...
TcpSocketBase::Send(Ptr<Packet> p, uint32_t flags)
{
 ...
 if (m_state == ESTABLISHED || m_state == SYN_SENT || m_state == CLOSE_WAIT)
 {
     ...
     m_rateOps->CalculateAppLimited(m_tcb->m_cWnd, //PPT
     ...
     { // Try to send the data out: Add a little step to allow the application
         ...
         if (!m_sendPendingDataEvent.IsPending())
         {
            ...
            if(m_tcb->lcpActive && m_tcb->lcpTurn) {
                m_sendPendingDataEvent = Simulator::Schedule(TimeStep(2),
                                                          &TcpSocketBase::SendPendingData,
                                                          this,
                                                          m_connected);
            }
 ...
 if (ppt)
   {
    ...
    // HCP uses 0–3, LCP uses 4–7
     ...
     {
        // NS_LOG_UNCOND ("[TcpPpt::CwndEvent] Low Priority Sent ");
         pptPri = idx + 4; // low‑priority loop
     }
     if(m_tcb->lcpActive) {
        m_tcb->lcpTurn = !m_tcb->lcpTurn;
     }
 }
 else
 {
     // Fallback to existing socket priority
     pptPri = GetPriority();
 }
 // Apply priority tag
 SocketPriorityTag priorityTag;
 priorityTag.SetPriority(pptPri);
 p->ReplacePacketTag(priorityTag);
}
...
TcpSocketBase::SendDataPacket(SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
...

 {
     ...
     if (m_tcb->m_pptBytesSent == 0)
     {
         m_tcb->m_pptLargeIdentified = (pktSize > TcpSocketState::PPT_LARGE_FLOW_THRESHOLD);
     }
     m_tcb->m_pptBytesSent += pktSize;
 }
...
TcpSocketBase::SendPendingData(bool withAck)
{
 ...
 while (availableWindow > 0)
 {
     ...
     bool success = false;
     if(m_tcb->lcpActive && m_tcb->lcpTurn) {
        if (!m_txBuffer->LastSeg(&next, &nextHigh, enableRule3))
        {
     ...
     else if (!m_txBuffer->NextSeg(&next, &nextHigh, enableRule3))
     {
         NS_LOG_INFO("no valid seq to transmit, or no data available");
         break;
     }
     ...
     if(success)
     {
         ...
         if(m_tcb->lcpActive && m_tcb->lcpTurn) {
            maxSizeToSend = static_cast<uint32_t>(next - nextHigh);
         }
